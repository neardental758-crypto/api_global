...
const finalizeLoan5g = async (req, res) => {
  const transaction = await sequelize.transaction();
  try {
    const { pre_id } = req.params;
    const { userId } = req.body;
    ...
    const soloHora = fechaOriginal.toISOString().split('T')[1].substring(0, 8);

    const otrosPrestamosActivos = await prestamosModels.findAll({
      where: {
        pre_usuario: prestamo.pre_usuario,
        pre_estado: {
          [Op.in]: ['ACTIVA', 'PRESTAMO', 'PRESTAMO PERSONALIZADO', 'PRESTAMO DE EMERGENCIA']
        },
        pre_id: {
          [Op.ne]: pre_id
        }
    ...
    res.status(200).json({
      success: true,
      message: 'Préstamo 5G finalizado exitosamente',
      data: {
        pre_id: pre_id,
        estado_prestamo: 'FINALIZADA',
        estado_bicicleta: 'DISPONIBLE',
    ...
    await transaction.rollback();
    console.error('Error al finalizar préstamo 5G:', error);

    res.status(500).json({
      success: false,
      message: 'Error interno del servidor al finalizar préstamo 5G',
      error: error.message
    });
  }
};
...
module.exports = {
  ...
  getItemsForReports, getItemsForReportsByOrganization, getItemsForReportsByStation, finalizeLoan, finalizeLoan4g, finalizeLoan5g
}
